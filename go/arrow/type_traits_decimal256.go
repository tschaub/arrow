...
func (decimal256Traits) CastFromBytes(b []byte) []decimal256.Num {
	...
	return unsafe.Slice((*decimal256.Num)(unsafe.Pointer(h.Data)), len(b)/Decimal256SizeBytes)
}

func (decimal256Traits) CastToBytes(b []decimal256.Num) []byte {
	h := (*reflect.SliceHeader)(unsafe.Pointer(&b))

	return unsafe.Slice((*byte)(unsafe.Pointer(h.Data)), len(b)*Decimal256SizeBytes)
}
